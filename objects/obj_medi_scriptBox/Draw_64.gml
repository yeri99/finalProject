...
if(instance_exists(obj_mediGameEvent) && obj_mediGameEvent.isPress && !global.IsMediGame){
...
else if(instance_exists(obj_mediGame)){
	if(obj_mediGame.isPress)
		draw_text(192,578,textGameSuccess)
	else if(!obj_mediGame.isPress && obj_mediGame.isKey){
		draw_text(192,578,textKey)
	}
	else if(obj_mediCabinetEvent.isPress)
...
else if(instance_exists(obj_twinkle) && obj_sscissorEvent.isPress){
	...
	instance_create_depth(room_width/2,room_height/2,depth,obj_bandage)
}
else{
	draw_text(192,578,textEndGame)
}